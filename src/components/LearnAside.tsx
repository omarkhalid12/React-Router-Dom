      ...
      <div className="h-full pb-4 overflow-y-auto">
        <nav className="space-y-2 font-medium">
          <ul className="space-y-3">
            ...
            {LEARN_NAV_ITEMS.map(({text, to}, index)=> (
              <li key={index}>
                <NavLink to={to} end className="flex items-center p-3 text-gray-900 rounded-r-lg dark:text-white hover:bg-gray-100 dark:hover:bg-[#283541] hover:text-[#149eca] duration-300">
                  <span>{text}</span>
                </NavLink>
              </li>
            ))}
